...
/*   Updated: 2024/12/16 18:13:59 by tle-goff         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	add_var(t_list **lst_var, char *command)
{
	...
	if (content[0] == '"' && content[ft_strlen(content) - 1] == '"')
		content = remove_double_quote(content);
	if (tab == -1)
...
void	cmd_format(char *command, t_list **lst_var, char **last_cmd, t_block **lst_block)
{
	...
	main_install_block(lst_block, &command[i]);
	if (check_equal("look", &command[i]))
		print(*lst_var, *last_cmd);
	else if (check_equal("clear", &command[i]))
		ft_lst_block_clear(lst_block);
	else if (check_equal("lookblock", &command[i]))
		print_block(*lst_block);
	else
	{
		if (command[0] != '\0')
		...
		if (check_equal("exit", &command[i]))
		{
			...
			echo_cmd(&command[i], lst_var, lst_block);

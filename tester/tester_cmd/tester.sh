...
run_test() {
    ...
    grep -v "ele-lean@c1r2p5" tmp_out_raw.txt > tmp_out.txt
    local actual_output
    actual_output=$(<tmp_out.txt)

    echo "${command}" | /bin/bash > tmp_out_bash.txt 2> /dev/null
    grep -v "ele-lean@c1r2p5" tmp_out_bash.txt > tmp_out_bash_filtered.txt
    local bash_output
    bash_output=$(<tmp_out_bash_filtered.txt)
...
#run_test "ls -lR sur le / avec wc" "ls -lR / | wc -l"
